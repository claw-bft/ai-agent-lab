...
class AICodeGenerator:
    ...
    def _call_ai_api(self, prompt: str, max_retries: int = 3) -> Optional[str]:
        """
        调用AI API生成代码
...
"""{request.prompt} - Generated by AI Code Generator"""
...
'''
    ...
    def _generate_setup_instructions(self, request: CodeGenerationRequest, architecture: Dict[str, Any]) -> List[str]:
        """生成安装说明"""
        return [
            f"1. Install dependencies: {self._get_install_command(request.language)}",
            f"2. Run the application: {self._get_run_command(request.language, architecture)}"
        ]
...
def main():
    ...
    parser.add_argument("--provider", default="claude", choices=["claude", "openai", "kimi"], 
                       help="AI provider (claude/openai/kimi)")
    ...
    if not api_key:
        print("⚠️  Warning: No AI API key found. Using template generation.")
        print("   Set ANTHROPIC_API_KEY, OPENAI_API_KEY, or KIMI_API_KEY for AI-powered generation.")
    ...
    if result.success:
        ...
        print(f"  Provider: {args.provider if api_key else 'template (no API key)'}")
