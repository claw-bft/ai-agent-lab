...
# 生成CLI工具
python ai_code_generator.py "创建一个文件批量重命名工具" --language python --output ./rename-tool

# 生成Web应用
python ai_code_generator.py "创建一个待办事项Web应用" --language typescript --framework react --output ./todo-app
```
...
└── ai_code_generator.py       # AI代码生成器 (新增)
```
...
```bash
# 代码生成 (基础版)
coding-pro generate --prompt "创建一个Python FastAPI用户认证服务" --output ./auth-service
